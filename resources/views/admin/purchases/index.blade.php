...
@section('content')
                                    ...
                                    <div class="col-sm-3 col-lg-2">
                                        ...
                                        <select name="type" id="search_type" class="form-select" required>
                                            ...
                                            <option value="product" {{ request('type') == 'product' ? 'selected' : '' }}>
                                                Product</option>
                                            <option value="supplier" {{ request('type') == 'supplier' ? 'selected' : '' }}>
                                                Supplier</option>
                                            <option value="reference_number"
                                                {{ request('type') == 'reference_number' ? 'selected' : '' }}>Reference #
                                            </option>
                                        </select>
                                    </div>
                                    <div class="col-sm-3 col-lg-2">
    ...
    @foreach ($purchases as $purchase)
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                ...
                                <div class="input-group">
                                    <input type="number" class="form-control payment-amount" name="amount" step="0.01"
                                ...
                                <small class="text-muted">Remaining: Rs <span
                                        class="remaining-amount-{{ $purchase->id }}">{{ number_format($remaining, 2) }}</span></small>
                            </div>
        ...
        @if ($purchase->payments->count() > 0)
            <div class="modal fade" id="paymentHistoryModal{{ $purchase->id }}" tabindex="-1"
                aria-labelledby="paymentHistoryModalLabel{{ $purchase->id }}" aria-hidden="true">
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="row mb-3">
                                ...
                                <div class="col-md-4">
                                    <strong>Total Paid:</strong> <span class="text-success">Rs
                                        {{ number_format($totalPaid, 2) }}</span>
                                </div>
                                <div class="col-md-4">
                                    <strong>Remaining:</strong> <span
                                        class="text-{{ $remaining > 0 ? 'warning' : 'success' }}">Rs
                                        {{ number_format($remaining, 2) }}</span>
                                </div>
                            </div>
...
@section('js')
    <script>
        ...
        function setMaxAmount(purchaseId) {
            const remaining = parseFloat(document.querySelector('.remaining-amount-' + purchaseId).textContent.replace(
                /[^0-9.-]+/g, ''));
